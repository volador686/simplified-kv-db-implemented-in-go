...
func (d *Database) SendReplica(replica_addr string) (string, string) {
	...
	d.db.View(func(tx *bolt.Tx) error {
		...
		for key, value := c.First(); key != nil; key, value = c.Next() {
			...
			// need a function to send key/value pairs to the replication server
			// due to different environment, this function can be different.
			// func () {}
			// and remember to delete the key-value pairs in replicaBucket.
		}
		return nil
	})
